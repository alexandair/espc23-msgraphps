...
function Get-MSCloudIdConsentGrantList {
    ...
    $tenantId = (Get-MgOrganization).ObjectId
    ...
    foreach ($sp in $servicePrincipals) {
        ...
        $spPermGrants = Get-MgServicePrincipalOauth2PermissionGrant -ObjectId $sp.ObjectId -All $true
    ...
    foreach ($grant in $Oauth2PermGrants) {
        if ($grant.ResourceId -eq $aadGraphSp.ObjectId -and $grant.Scope) {
            $grant.Scope.Split(" ") | Where-Object { $_ } | ForEach-Object {
                ...
                {
                    $ownerUPN = (Get-MgServicePrincipalOwner -ObjectId $client.ObjectId -Top 1).UserPrincipalName
    ...
    $script:ObjectByObjectClassId['ServicePrincipal'].GetEnumerator() | ForEach-Object {
        ...
        Get-MgServicePrincipalAppRoleAssignedTo -ObjectId $sp.ObjectId -All $true `
